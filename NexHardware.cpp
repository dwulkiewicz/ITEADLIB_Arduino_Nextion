...
*/
bool nexInit(int dbBaud, unsigned long nexBaud, uint32_t config, int8_t rxPin, int8_t txPin)
{
    bool ret1 = false;
    bool ret2 = false;
    
    dbSerialBegin(dbBaud);

    nexSerial.begin(nexBaud, config, rxPin, txPin);
    ...
    {   
        ...
        if (NEX_RET_CURRENT_PAGE_ID_HEAD == c)
        {
            if (nexSerial.available() >= 4)
            {
                ...
                {
                    NexTouch::iterate(nex_listen_list, __buffer[1], 0, 0xFF);
                }                
            }
        }		
    }
