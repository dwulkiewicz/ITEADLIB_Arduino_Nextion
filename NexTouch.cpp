    ...
    :NexObject(pid, cid, name)
{
    ...
    this->__cb_onShow = NULL;
    this->__cbonShow_ptr = NULL;	
}
...
void NexTouch::attachOnShow(NexTouchEventCb onShow, void *ptr)
{
    this->__cb_onShow = onShow;
    this->__cbonShow_ptr = ptr;	
}
void NexTouch::detachOnShow(void)
{
    this->__cb_onShow = NULL;
    this->__cbonShow_ptr = NULL;		
}
...
void NexTouch::onShow(void)
{
    if (__cb_onShow)
    {
        __cb_onShow(__cbonShow_ptr);
    }
}
...
void NexTouch::iterate(NexTouch **list, uint8_t pid, uint8_t cid, int32_t event)
{
    ...
    for(i = 0; (e = list[i]) != NULL; i++)
    {
        if (e->getObjPid() == pid && e->getObjCid() == cid)
        {
            ...
            else if (NEX_EVENT_ON_SHOW == event)
            {
                e->onShow();
            }            
            break;
